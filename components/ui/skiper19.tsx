...
const Skiper19 = () => {
  ...
  const [query, setQuery] = useState(""); // Import useState
    ...
    >
      <div className="mt-42 relative flex w-fit flex-col items-center justify-center gap-5 text-center">
        ...
        <p className="font-jakarta-sans relative z-10 max-w-2xl text-xl font-medium text-[#1F3A4B]">
          Your accelerated path to knowledge, Scroll down to start searching.
        </p>
        ...
        <div className="mt-12 w-full max-w-2xl px-4">
          <form onSubmit={handleSearch} className="w-full relative group">
            <input
              ...
              className="w-full pl-6 pr-32 py-6 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-lg text-white placeholder-white/50 focus:outline-none focus:bg-white/20 transition-all font-medium"
            />
            <button
              type="submit"
              className="absolute inset-y-2 right-2 px-8 bg-[#C2F84F] text-[#1F3A4B] rounded-full font-bold text-base hover:bg-[#b0e640] transition-colors"
            >
              Search
            </button>
          </form>
